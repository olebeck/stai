...
static int sce_exe_alloc(SceUID pid, SceUID* exec_uid, SceUID* write_uid, uintptr_t* exec_ptr, void** write_ptr, size_t align,size_t size) {
  ...
  *write_ptr = NULL;
  ...
  opt.alignment = align;
  opt.pid = pid;
  ret = ksceKernelAllocMemBlock("exec_mem", SCE_KERNEL_MEMBLOCK_TYPE_USER_MAIN_RX, size, &opt);
  if(ret < 0) {
...
void *slab_alloc(struct slab_chain *const sch, uintptr_t *exe_addr)
{
  ...
  if (LIKELY(sch->partial != NULL)) {
    ...
    *exe_addr = sch->partial->exe_data;
    ...
    uintptr_t exe_data;
    int ret = sce_exe_alloc(sch->pid, &exe_uid, &write_uid, &exe_data, (void **)&sch->partial, sch->slabsize, sch->pages_per_alloc);
    if (ret < 0) {
      *exe_addr = 0;
      sch->partial = NULL;
      return NULL;
    }
    sch->partial->write_uid = write_uid;
    sch->partial->exe_uid = exe_uid;
